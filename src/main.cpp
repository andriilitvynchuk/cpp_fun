...
// Class template: e.g. buffer of any element type (like cv::Mat channels, or landmark type in SLAM)
template <typename T>
class Buffer {
public:
...
// Two main tools: exceptions (for "this should never happen" / unrecoverable) and
// std::optional / std::expected (for "no result" that callers can handle).

// Custom exception: useful for domain errors (e.g. SLAM: tracking lost, no pose).
...
// Return optional when "no result" is a valid outcome (e.g. keyframe not found).
...
public:
    ...
    void ranges_examples() {
        ...
        auto result = vector |
                      std::ranges::views::filter([](int& element) { return element % 2 == 0; }) |
